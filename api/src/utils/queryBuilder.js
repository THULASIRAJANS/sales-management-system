class QueryBuilder {
  ...
  addDateFilter(dateFilter) {
    ...
    console.log('📅 Date Filter Received:', JSON.stringify(dateFilter, null, 2));
      ...
      quickSelect.forEach(option => {
        ...
        switch (option) {
          case 'today':
            ...
            break;
          case 'yesterday':
            dateConditions.push('DATE(t.date) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)');
            break;
          case 'last7days':
            dateConditions.push('DATE(t.date) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)');
            break;
          case 'last30days':
            dateConditions.push('DATE(t.date) >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)');
            break;
          case 'thisMonth':
            dateConditions.push('MONTH(t.date) = MONTH(CURDATE()) AND YEAR(t.date) = YEAR(CURDATE())');
            break;
          case 'lastMonth':
            dateConditions.push('MONTH(t.date) = MONTH(DATE_SUB(CURDATE(), INTERVAL 1 MONTH)) AND YEAR(t.date) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 MONTH))');
            break;
          default:
    ...
    // Handle custom date - Format: "day/month/year"
    if (customDate && customDate.trim() !== '') {
      ...
      if (parts.length === 3) {
        ...
        if (isNaN(monthInput)) {
          ...
          console.log(`🔄 Converted month name "${monthInput}" to number:`, monthNumber);
        } else {
        ...
        console.error('❌ Invalid custom date format. Expected 3 parts, got:', parts.length);
      }
    }
